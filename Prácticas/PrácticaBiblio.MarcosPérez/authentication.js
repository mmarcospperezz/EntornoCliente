...
function login() {
    ...
    // Obtenemos los usuarios actualizados del localStorage
    const usuarios = JSON.parse(localStorage.getItem('usuarios'));

    const found = usuarios.find(u => u.user === user && u.pass === pass);

    if (found) {
        // 1. Guardamos en variable global
        usuarioLogueado = found; 
        // 2. Guardamos en el almacenamiento de sesión
        sessionStorage.setItem('sesionActive', JSON.stringify(found));
        
        // 3. ¡IMPORTANTE! Actualizamos la interfaz
        checkUI(); 
    } else {
